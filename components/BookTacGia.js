...
export default function BookTacGia({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    fetchAuthor(author.id, 1);
  }, [author.id]);

  const fetchAuthor = async (authorID, page = 1) => {
    try {
      if (!authorID) return;

      setLoading(true);

      const res = await authorTruyen(authorID, page);
  ...
  const loadMore = () => {
    ...
    setPage(nextPage);
    fetchAuthor(author.id, nextPage);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <HeaderLight textTitle={'Tác Giả: '+author.name} link={''} />
